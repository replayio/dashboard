    ...
    >
      {steps.map((step, index) => {
        ...
        return (
          <Fragment key={index}>
            <div
              className={`shrink-0 flex flex-rol items-center gap-2 ${isCurrent || isCompleted ? "text-foreground" : "text-muted-foreground"}`}
              data-test-name={`MultiStepForm-Step-${index + 1}`}
              data-test-state={isCurrent ? "current" : isCompleted ? "complete" : "incomplete"}
            >
              {isCompleted ? (
                <Icon className="w-6 h-6 text-blue-400" type="step-complete" />
              ) : (
                <div className="relative flex items-center justify-center w-6 h-6">
                  <Icon className="absolute w-6 h-6" type="step-incomplete" />
                  <span className="relative text-xs text-foreground">{index + 1}</span>
                </div>
              )}
              <span>{step}</span>
            </div>

            {index < steps.length - 1 && <div className="w-full h-px mx-4 grow bg-border" />}
          </Fragment>
        );
      })}
    </div>
